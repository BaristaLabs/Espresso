namespace BaristaLabs.Espresso.Engine.V8.Tests
{
    ...
    [TestClass]
    [DeploymentItem("BaristaLabs.Espresso.Engine.V8.dll")]
    [DeploymentItem("BaristaLabs.Espresso.Engine.V8.Native.dll")]
    ...
    public class V8ScriptEngineTest
    {
        ...
        public void V8ScriptEngine_Evaluate_WithDocumentName()
        {
            ...
            ((V8ScriptEngine)engine).EnableDocumentNameTracking();
            Assert.AreEqual(Math.E * Math.PI, engine.Evaluate(documentName, "Math.E * Math.PI"));
            Assert.IsFalse(((V8ScriptEngine)engine).GetDocumentNames().Any(name => name.StartsWith(documentName, StringComparison.Ordinal)));
        }
        ...
        public void V8ScriptEngine_Evaluate_DiscardDocument()
        {
            const string documentName = "DoTheMath";
            ((V8ScriptEngine)engine).EnableDocumentNameTracking();
            Assert.AreEqual(Math.E * Math.PI, ((V8ScriptEngine)engine).Evaluate(documentName, true, "Math.E * Math.PI"));
            Assert.IsFalse(((V8ScriptEngine)engine).GetDocumentNames().Any(name => name.StartsWith(documentName, StringComparison.Ordinal)));
        }
        ...
        public void V8ScriptEngine_Evaluate_RetainDocument()
        {
            const string documentName = "DoTheMath";
            ((V8ScriptEngine)engine).EnableDocumentNameTracking();
            Assert.AreEqual(Math.E * Math.PI, ((V8ScriptEngine)engine).Evaluate(documentName, false, "Math.E * Math.PI"));
            Assert.IsTrue(((V8ScriptEngine)engine).GetDocumentNames().Any(name => name.StartsWith(documentName, StringComparison.Ordinal)));
        }
        ...
        public void V8ScriptEngine_Execute_DiscardDocument()
        {
            const string documentName = "DoTheMath";
            ((V8ScriptEngine)engine).EnableDocumentNameTracking();
            ((V8ScriptEngine)engine).Execute(documentName, true, "epi = Math.E * Math.PI");
            Assert.AreEqual(Math.E * Math.PI, engine.Script.epi);
            Assert.IsFalse(((V8ScriptEngine)engine).GetDocumentNames().Any(name => name.StartsWith(documentName, StringComparison.Ordinal)));
        }
        ...
        public void V8ScriptEngine_Execute_RetainDocument()
        {
            const string documentName = "DoTheMath";
            ((V8ScriptEngine)engine).EnableDocumentNameTracking();
            ((V8ScriptEngine)engine).Execute(documentName, false, "epi = Math.E * Math.PI");
            Assert.AreEqual(Math.E * Math.PI, engine.Script.epi);
            Assert.IsTrue(((V8ScriptEngine)engine).GetDocumentNames().Any(name => name.StartsWith(documentName, StringComparison.Ordinal)));
        }
        ...
        public void V8ScriptEngine_Script_Property()
        {
            ...
            engine.Script.myObj = myObj;
            Assert.AreSame(myObj, engine.Script.myObj);
        }
        ...
        public void V8ScriptEngine_MaxRuntimeHeapSize_ShortBursts()
        {
            ...
            engine.MaxRuntimeHeapSize = (UIntPtr)limit;
            ((V8ScriptEngine)engine).RuntimeHeapSizeSampleInterval = TimeSpan.FromMilliseconds(30000);
        ...
        //        {
        //            innerEngine.Script.test = new Action(() => Assert.AreSame(innerEngine, V8ScriptEngine.Current));
        //            Assert.AreSame(engine, V8ScriptEngine.Current);
        //            innerEngine.Execute("test()");
        //            innerEngine.Script.test();
        //            Assert.AreSame(engine, V8ScriptEngine.Current);
        //        });

        //        Assert.IsNull(V8ScriptEngine.Current);
        //        engine.Execute("test()");
        //        engine.Script.test();
        //        Assert.IsNull(V8ScriptEngine.Current);
        //    }
