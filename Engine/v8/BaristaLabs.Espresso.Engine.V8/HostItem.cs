...
namespace BaristaLabs.Espresso.Engine.V8
{
    ...
    internal partial class HostItem : DynamicObject, IExpando, IDynamic, ICustomQueryInterface, IScriptMarshalWrapper, IHostInvokeContext
    {
        #region data
        ...
        private HostItem(V8ScriptEngine engine, HostTarget target, HostItemFlags flags)
        {
            this.engine = engine;
            this.target = target;
            this.flags = flags;
        ...
        public static object Wrap(V8ScriptEngine engine, object obj, Type type)
        {
            return Wrap(engine, obj, type, HostItemFlags.None);
        }

        public static object Wrap(V8ScriptEngine engine, object obj, HostItemFlags flags)
        {
            return Wrap(engine, obj, null, flags);
        }

        private static object Wrap(V8ScriptEngine engine, object obj, Type type, HostItemFlags flags)
        {
        ...
        public delegate HostItem CreateFunc(V8ScriptEngine engine, HostTarget target, HostItemFlags flags);
        ...
        private static object BindOrCreate(V8ScriptEngine engine, object target, Type type, HostItemFlags flags)
        {
            return BindOrCreate(engine, HostObject.Wrap(target, type), flags);
        }

        private static object BindOrCreate(V8ScriptEngine engine, HostTarget target, HostItemFlags flags)
        {
            return engine.GetOrCreateHostItem(target, flags, Create);
        }

        private static HostItem Create(V8ScriptEngine engine, HostTarget target, HostItemFlags flags)
        {
        ...
        {
            var newAccessContext = flags.HasFlag(HostItemFlags.PrivateAccess) ? target.Type : null;
