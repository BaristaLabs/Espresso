    ...
    internal partial class HostItem
    {
        ...
        private MethodBindResult BindMethod(string name, Type[] typeArgs, object[] args, object[] bindArgs)
        {
            ...
            else
            {
                ...
                //NEVER use reflection bind fallback
        ...
        {
            var expr = DynamicExtensions.Bind(binder, target, bindArgs).Expression;
            if (expr == null)
            {
        ...
        private sealed class MethodBindingVisitor : ExpressionVisitor
        {
            ...
            protected override Expression VisitInvocation(InvocationExpression node)
            {
                ...
                {
                    var del = DynamicExtensions.Invoke(node.Expression) as Delegate;
            ...
            protected override Expression VisitUnary(UnaryExpression node)
            {
                ...
                {
                    var exception = DynamicExtensions.Invoke(node.Operand) as Exception;
                    if (exception != null)
                    {
                        AddResult(() => (Exception)DynamicExtensions.Invoke(node.Operand));
                    }
                }
