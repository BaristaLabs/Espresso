...
namespace BaristaLabs.Espresso.Engine.V8
{
    ...
    {
        public static Delegate CreateProc(V8ScriptEngine engine, object target, int argCount)
        {
        ...
        public static Delegate CreateFunc<TResult>(V8ScriptEngine engine, object target, int argCount)
        {
            if ((argCount < 0) || (argCount > maxArgCount))
            {
                throw new ArgumentException("Invalid argument count", "argCount");
        ...
        public static TDelegate CreateDelegate<TDelegate>(V8ScriptEngine engine, object target)
        {
            return (TDelegate)(object)CreateDelegate(engine, target, typeof(TDelegate));
        }

        public static Delegate CreateDelegate(V8ScriptEngine engine, object target, Type delegateType)
        {
        ...
        private static Delegate CreateSimpleDelegate(V8ScriptEngine engine, object target, Type delegateType)
        {
            var method = delegateType.GetMethod("Invoke");
        ...
        private static Delegate CreateComplexDelegate(V8ScriptEngine engine, object target, Type delegateType)
        {
        ...
        private abstract class DelegateShim
        {
            ...
            protected DelegateShim(V8ScriptEngine engine)
            {
                Engine = engine;
            }
        ...
        private abstract class ProcShim : DelegateShim
        {
            ...
            protected ProcShim(V8ScriptEngine engine)
                : base(engine)
            {
                if (engine == null)
                {
        ...
        private abstract class FuncShim<TResult> : DelegateShim
        {
            ...
            protected FuncShim(V8ScriptEngine engine)
                : base(engine)
            {
                if (engine == null)
                {
